...
app.controller("solutionmanagerCtrl",function($uibModal,$log,$scope,$state,$http,factory_cookie,jsonToStr,Upload){
	var ctrl = this;
	...
	$scope.myCookie = {
		...
		firsttype:factory_cookie.getCookie("firsttype"),
		secondtype:factory_cookie.getCookie("secondtype")
	};
	if ($scope.myCookie.firsttype == null || $scope.myCookie.secondtype == null) {
		alert("非法登录！");
		$state.go("login");
	}
	...
	$scope.solutionmanager_refresh = function() {
		$state.reload("workspace.solutionmanager");
	}
	//每页显示个数
...
//发起项目 addproject
app.controller("modal_addprojectCtrl",function($uibModalInstance,p,$scope,$state,$http,factory_cookie,jsonToStr){
	var ctrl = this;
	ctrl.p = p; //同步p参数
