...
app.controller("solutionmanagerCtrl_2_2",function($filter,$uibModal,$log,$scope,$state,$http,factory_cookie,jsonToStr,Upload){
	var ctrl = this;
	...
	$scope.search = {
		show:true,
		s_projectname:"", //合同名称查询
		s_projectMintime:"", //合同时间开始
		s_projectMaxtime:"", //合同时间结束
		s_timeIsOpend_min:false, //时间控件关闭
		s_timeIsOpend_max:false, //时间控件关闭
	...
	//时间控件初始化
	$scope.dateOptions_min = {
	    maxDate: $scope.search.s_projectMaxtime!=""?new Date($scope.search.s_projectMaxtime):new Date(),
	    startingDay: 1
  	};
  	$scope.dateOptions_max = {
	    minDate: $scope.search.s_projectMintime!=""?new Date($scope.search.s_projectMintime):"",
	    maxDate: new Date(),
	    startingDay: 1
  	};
  	$scope.dateOptions_tab_info = {
	    maxDate: new Date(),
	    startingDay: 1
  	};
	...
	$scope.solutionmanager_refresh = function() {
		$state.reload("workspace.solutionmanager_2_2");
	}
	//每页显示个数
	...
	//新增弹窗
	ctrl.modal_addproject = function() {
		ctrl.p = {
			firsttype:2,
			secondtype:2
		};
		...
		var modalInstance = $uibModal.open({
			...
			ariaDescribedBy:"modal-body",
			ariaLabelledBy:"modal-title",
			templateUrl:"modal_addproject",
			controller:"modal_addprojectCtrl",
			controllerAs:"ctrl",
			//size:"sm",
			resolve:{
				p:function(){
		...
		modalInstance.result.then(function(data){
			if(data == 1) {
				$scope.work.modal_Show("提示","项目创建成功！","sm");
				$scope.search.s_pageNum = 1;
			}
			ko();
		},function(){
			$log.info('Modal cancel at: ' + new Date());
	...
	function send_modifyproject() {
		...
		var t2 = {
			...
			projecttime:$filter('date')($scope.tab_info.info.projecttime,'yyyy-MM-dd')
		};
	...
	function send_addOrDeletePerson(p) {
		if (flag_projectID == null) {
			alert("信息错误，将重新加载页面！");
			$state.reload("workspace.solutionmanager_2_0");
		}
	...
	function send_searchproject(p) {
		...
		var t2 = {
			...
			firsttype:2,
			secondtype:2,
			projectMintime:$filter('date')($scope.search.s_projectMintime,'yyyy-MM-dd'),
			projectMaxtime:$filter('date')($scope.search.s_projectMaxtime,'yyyy-MM-dd')
		};
